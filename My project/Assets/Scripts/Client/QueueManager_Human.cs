...
public class QueueManager_Human : MonoBehaviour
{
    ...
    // Spawn Human customers only
    void SpawnCustomer()
    {
        if (customerPrefabs.Length == 0 || spawnPoint == null) return;

        // Keep spawning until we get a Human type customer
        CustomerOrderSystem customer = null;

        while (customer == null || customer.customerType != CustomerOrderSystem.CustomerType.Human)
        {
            ...
            if (customer == null || customer.customerType != CustomerOrderSystem.CustomerType.Human)
            {
                if (newCustomer != null)
                    Destroy(newCustomer);
                customer = null;
    ...
    public void CustomerLeft(CustomerOrderSystem customer)
    {
        ...
        {
            Debug.Log($"Human customer left. Remaining: {queue.Count - 1}");
    ...
    void UpdateQueue()
    {
        Debug.Log($"=== Updating Human queue. Count: {queue.Count} ===");
        ...
        for (int i = 0; i < queue.Count; i++)
        {
            ...
            if (i == 0)
            {
                ...
                Debug.Log($"Human Customer {i}: assigned to counter");
            }
            else if (i - 1 < queuePositions.Length)
            {
                queue[i].queuePosition = queuePositions[i - 1];
                Debug.Log($"Human Customer {i}: assigned to {queuePositions[i - 1].name}");
            }
            else
            {
                Debug.LogWarning($"Human Customer {i}: too many! Using last position.");
